...
#
"""
main disassembler module. From here we call various version-specific
...
class Disassemble(object):
    ...
    def __init__(self, version):
        ...
        self.resetInstructionClass()

    def setInstructionClass(self, tokenClass):
        # assert isinstance(tokenClass, types.ClassType)
        self.Instruction = tokenClass
        return self.Instruction

    def resetInstructionClass(self):
        return self.setInstructionClass(Instruction)
...
def get_disasm(version):
    ...
    if version == 2.7:
        ...
        import pyxdis.disasm.disasm31 as disassemble
        disasm = disassemble.Disassemble31()
    elif version == 3.2:
        import pyxdis.disasm.disasm32 as disassemble
        disasm = disassemble.Disassemble32()
    elif version == 3.3:
        import pyxdis.disasm.disasm33 as disassemble
        disasm = disassemble.Disassemble33()
    elif version == 3.4:
        import pyxdis.disasm.disasm34 as disassemble
        disasm = disassemble.Disassemble34()
    elif version == 3.5:
        import pyxdis.disasm.disasm35 as disassemble
        disasm = disassemble.Disassemble35()
    else:
        raise RuntimeError("Unsupported Python version %d" % version)
    return disasm

if __name__ == "__main__":
    import inspect, pyxdis
    co = inspect.currentframe().f_code
    disasm = get_disasm(pyxdis.PYTHON_VERSION)
    instructions = disasm.disassemble(co, {})
    print('-' * 30)
